...
export default function Programcilar() {
  ...
  const [enIyi, setEnIyi] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    let selected = enIyiler.find(element => element.id === enIyi);
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: enIyi !== null ? 'gold' : 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          enIyiler.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {/* burada dev.id 'yi öne çıkan id'ye atayın */ setEnIyi(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
